...
def findUnitClause(clauses, model):
    ...
    for clause in clauses:
        ...
        if type(clause) is int:
            if clause > 0: return clause, True
            else: return -1 * clause, False
        else:
...
def DPLL(clauses, symbols, model):
    if checkEarlyTerminationTrue(clauses, model): return True, model
    if checkEarlyTerminationFalse(clauses, model): return False, model
    ...
    if P:
        ...
        tempModel[P] = value
        return DPLL(clauses, temp, tempModel)
    ...
    modelTrue[P] = True
    modelFalse[P] = False
    return DPLL(clauses, copyList, modelTrue) or DPLL(clauses, copyList, modelFalse)


def satSolver(file):
    clauses, symbols = getClausesAndSymbols(file)
    result, model = DPLL(clauses, symbols, {})
    return result, model


testfile = 'Project5_testcases/testcase-quinn.txt'
print(satSolver(testfile))